...
SimpleApp::SimpleApp(const std::string &content, const std::string &step) {
    m_content = content;
    m_step = step;
}
...
void SimpleApp::OnContextInitialized() {
  ...
  if (use_views) {
    ...
    CefRefPtr<CefFrame> frame = SimpleHandler::GetInstance()->getBrowser()->GetMainFrame();
    std::string jsCode=R"(var custom_generate_content = ')" + m_content + R"(';)" + R"(var m_step = )" + m_step + R"(;)" + R"(console.log("custom_generate_content=",custom_generate_content);)" + R"(console.log("m_step=",m_step);)" ;
