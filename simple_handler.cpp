...
bool SimpleHandler::OnConsoleMessage(CefRefPtr<CefBrowser> browser,
                      ...
                      int line) {
    std::cout << "Console Message: " << message.ToString() << std::endl;

    // 假设 message 是 "eventType data"
    std::string messageStr = message.ToString();
    std::istringstream iss(messageStr);
    std::string eventType;
    std::string dataStr;

    // 使用空格分隔 eventType 和 data
    std::getline(iss, eventType, ' ');
    std::getline(iss, dataStr);

    // 将字符串转换为 CefString
    CefString cefEventType = eventType.c_str();
    CefString cefDataStr = dataStr.c_str();

    // 从 CefString 解析数据
    // 这里需要根据实际的数据格式来解析
    // 例如，如果 data 是一个 JSON 字符串，你可以使用 CefParseJSON 来解析
    std::cout << "cefEventType: " << cefEventType.ToString() << std::endl;
    if (cefEventType == "GENERATE_PPT_SUCCESS") {
        qDebug()<<QString::fromStdString(cefEventType.ToString());
    }
    return 1;
}
